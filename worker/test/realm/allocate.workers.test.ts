...
async function allocate(
  realmId: string,
  body: { operation_id: string; realm_secret64lowerhex: string; location_hint?: string },
...
function expectAccepted(response: { status: number; json: Json }, realmId: string, op: string) {
  ...
  expect(response.json).not.toHaveProperty("realm_secret64lowerhex");
}
...
describe("deployment allocation ingress", () => {
  it("acknowledges the first allocation and returns the exact result on a retry after the response is discarded", async () => {
    const realm = realmName();
    const operation = operationId();
    const secret = realmSecret();
    const request = { operation_id: operation, realm_secret64lowerhex: secret };

    const first = await allocate(realm, request);
    expectAccepted(first, realm, operation);
    await new Response(JSON.stringify(first.json)).arrayBuffer();

    const retry = await allocate(realm, request);
    expect(retry.status).toBe(200);
    expect(retry.json).toEqual(first.json);
    expect(retry.json).not.toHaveProperty("realm_secret64lowerhex");
  });
  ...
  it("refuses a different operation and a conflicting secret without changing the allocation", async () => {
    ...
    const differentOperation = await allocate(realm, { operation_id: operationId(), realm_secret64lowerhex: secret });
    expectConflict(differentOperation);
    expect(await snapshot(realm)).toEqual(before);

    const conflictingSecret = await allocate(realm, { operation_id: operation, realm_secret64lowerhex: realmSecret() });
  ...
  it("refuses allocation into a realm created through the legacy create route", async () => {
    ...
    const response = await allocate(realm, { operation_id: operationId(), realm_secret64lowerhex: realmSecret() });
  ...
  it("rejects missing, wrong, and realm-secret bearer authorization before allocation", async () => {
    const realm = realmName();
    const request = { operation_id: operationId(), realm_secret64lowerhex: realmSecret() };
    const before = await snapshot(realm);
    ...
    expect(await snapshot(realm)).toEqual(before);
    expectUnauthorized(await allocate(realm, request, request.realm_secret64lowerhex));
    expect(await snapshot(realm)).toEqual(before);
  });
  ...
  it("authenticates the supplied secret and never persists its plaintext", async () => {
    ...
    const secret = realmSecret();
    const operation = operationId();
    const response = await allocate(realm, { operation_id: operation, realm_secret64lowerhex: secret, location_hint: "weur" });
  ...
  it("rejects malformed secret material instead of treating a non-hex value as an allocation credential", async () => {
    const realm = realmName();
    const before = await snapshot(realm);
    const response = await allocate(realm, { operation_id: operationId(), realm_secret64lowerhex: "not-a-lowercase-hex-secret" });
