...
output_dir="${RESTORE_WIRE_CONTINUE_OUTPUT:-$runner_dir/tmp/provider-restore-wire-2}"
...
inputs=(
  ...
  "$worker_dir/run-restore-wire-continue.sh"
)
...
printf '%s\n' "timeout --signal=TERM --kill-after=5s 270s: vitest do (restore-wire)" >"$output_dir/command.txt"
