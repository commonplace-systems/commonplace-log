...
output_dir="${RESTORE_WIRE_CONTINUE_2_OUTPUT:-$runner_dir/tmp/provider-restore-wire-3}"
...
inputs=(
  ...
  "$worker_dir/run-restore-wire-continue-2.sh"
)
...
printf '%s\n' "timeout --signal=TERM --kill-after=5s 270s: vitest do (two anchored restore-wire failures)" >"$output_dir/command.txt"
