...
if [ "$v2_count" -lt 1 ] || [ "$v2_with_operation_id_count" -ne "$v2_count" ]; then
  echo "FAIL: valid v2 coverage invalid (v2=${v2_count}, with required operation_id=${v2_with_operation_id_count})"
  fail=1
else
  echo "v2 entry coverage: ${v2_count} total (all ${v2_with_operation_id_count} with required operation_id)"
fi
...
echo "v2 entries:                             ${v2_count} (all ${v2_with_operation_id_count} with required operation_id)"
...
echo "SELECTOR: green means: TS==Elixir==input over ${case_count} random I-JSON values — including ${v2_count} valid v2 entry shapes (all ${v2_with_operation_id_count} with required operation_id), null/true/false, integers within ±(2^53−1), finite doubles from random 64-bit patterns (subnormals/extremes included), well-formed-Unicode strings (controls, quotes, backslashes, astral), UTF-16-ordering-stress keys (BMP ≥ U+E000 mixed with astral), nesting to depth ~6; NOT covered: integers beyond ±(2^53−1), lone surrogates, duplicate keys (recorded-unpinned classes — see conformance/README.md), and everything invalid-entries covers."
