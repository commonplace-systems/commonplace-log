...
expected = json.loads((packet / 'INPUTS.json').read_text())
...
try:
    for label, expected_rc, total, failures, excluded in [('baseline', 1, 10, 1, 9), ('candidate', 0, 35, 0, 0)]:
        local = out / label
        local.mkdir(mode=0o700)
        stdout = child(label, [elixir, '-pa', beam, str(packet / 'check.exs'), str(local), label], app, 75, expected_rc)
        result = json.loads((local / 'test-result.json').read_text())
        assert result['total'] == total and result['failures'] == failures and result['excluded'] == excluded and result['skipped'] == 0, 'test acceptance'
        metrics = json.loads((out / (label + '-metrics.json')).read_text())
        assert [x['history_rows'] for x in metrics] == [64, 128] and all(x['prepared_equal'] for x in metrics), 'nonvacuous performance cases'
        if label == 'baseline':
            assert 'absent operation avoids repeated deep candidate construction' in stdout, 'named baseline failure'
            assert all(x['candidate']['reductions'] >= x['baseline']['reductions'] / 2 for x in metrics), 'baseline performance RED'
        else:
            assert all(x['candidate']['reductions'] < x['baseline']['reductions'] / 2 for x in metrics), 'candidate reduction improvement'
