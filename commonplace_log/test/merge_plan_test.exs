defmodule Commonplace.Log.MergePlanTest do
  ...
  defp mk(writer_id, writer_seq, entry_id, prev_entry_id, payload \\ "") do
    %{
      entry_id: entry_id,
      writer_id: writer_id,
  ...
  # else by equality. Failures re-assert with the full outcome for a readable
  # diff.
  ...
  defp w1, do: mk(@w, 1, "id-w1", nil)
  defp w2, do: mk(@w, 2, "id-w2", "id-w1")
  defp w3, do: mk(@w, 3, "id-w3", "id-w2")

  # --- the table -----------------------------------------------------------

  # Each row: {name, present-maker, batch-maker, expected outcome (partial)}.
  # Row bodies are built inside the test (fixtures are functions), so the
  # table itself holds zero-arg builders.
  defp rows do
    [
      {
  ...
  describe "plan_merge classifies batches per spec §9.3" do
    ...
    test "every table row classifies as expected" do
      for {name, present, batch, expected} <- rows() do
        ...
        unless matches?(outcome, expected) do
          flunk("""
          row failed: #{name}
          expected (partial): #{inspect(expected, pretty: true)}
          actual: #{inspect(outcome, pretty: true)}
          """)
        end
      end
    end
