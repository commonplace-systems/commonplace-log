...
- [ ] HARNESS FINDING (Task 7, verified independently in review): vitest-pool-workers 0.12.x re-addresses DOs by derived unique ids, so `ctx.id.name` is always undefined in tests even via `idFromName` — end-to-end named-addressing verification (a real `idFromName` stub.fetch seeing `ctx.id.name === "<log_id>"`) is deferred to SP4 wrangler-dev; in-test routes drive `handleRequest` with `objectName` injected over real DO SQLite, and the fetch-level tests prove the DO wiring via the genuine unnamed-object §13.1 rejection
