...
| SP4 | **[REPLACED per proposal §9/§11 Phase 3]** Cloudflare realm sidecar: `RealmContainer` DO (multi-log realm schema, per-log revision, read-set/commit endpoints, `storage.internal` outbound handler), `Commonplace.Log.Persistence.CloudflareSidecar`; optional `LogStore.WorkalikeClient` only if a deployment needs direct DO-workalike sync. **Planned 2026-08-23: `plans/2026-08-23-sp4-cloudflare-realm-sidecar.md`, split 4a (locally verifiable) / 4b (needs jes's account + Containers access)** | proposal §§6–8 | Same Engine tests green over LocalSQLite AND the sidecar; deployment conformance (proposal §12) |
