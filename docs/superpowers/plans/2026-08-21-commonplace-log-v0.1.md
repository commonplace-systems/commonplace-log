...
| SP3 | **[AMENDED by the BEAM-native revision, `docs/proposals/2026-08-22-beam-native-revision.md`]** Normative Elixir engine + persistence + sync: `Commonplace.Log.Engine` over the `Commonplace.Log.Persistence` behavior, `LocalSQLite` adapter, `Commonplace.Log.Sync` | §10, §12, §14, proposal §§5–6 | §18 1–12 green over Engine+LocalSQLite; both-order convergence on this box. (Original row's DO↔Elixir live demo moved to SP4, where its client dependency lives) |
| SP4 | **[REPLACED per proposal §9/§11 Phase 3]** Cloudflare realm sidecar: `RealmContainer` DO (multi-log realm schema, per-log revision, read-set/commit endpoints, `storage.internal` outbound handler), `Commonplace.Log.Persistence.CloudflareSidecar`; optional `LogStore.WorkalikeClient` only if a deployment needs direct DO-workalike sync. **SP4a COMPLETE 2026-08-23** (`4ea6e4e`, `7c43530`, `9ce7af0`, `5d680ab`, `73caa5f`, `a8752e4`, `a1b1201`); **SP4b awaits jes — see `docs/sp4b-deployment-readiness.md`** | proposal §§6–8 | **MET:** same Engine behaviour green over LocalSQLite, in-memory AND the sidecar, asserted pairwise-equal and demonstrated able to fail against three broken adapters. Deployment conformance (§12) partially deferred: `storage_full` never exercised, two concurrent Containers never occurred |
