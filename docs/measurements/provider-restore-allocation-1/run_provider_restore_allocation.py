#!/usr/bin/env python3
"""Bounded Vitest runner for deployment realm allocation."""
...
ALLOCATION_SOURCE = "5f9e810665ba3a52085dcde9b02e53ddbc180fa5"
FIXTURE_COMMIT = "cb490e6158e9011ea9d3193c0aa00eead457e5a5"
FIXTURE_SOURCE_COMMIT = "cb490e6158e9011ea9d3193c0aa00eead457e5a5"
TEST_FILE = "worker/test/realm/allocate.workers.test.ts"
PACKET_README = "docs/measurements/provider-restore-allocation-1/README.md"
PACKET_RUNNER = "docs/measurements/provider-restore-allocation-1/run_provider_restore_allocation.py"
FIXTURE_FULL_NAMES = (
...
if len(sys.argv) != 2:
    raise SystemExit("usage: run_provider_restore_allocation.py /absolute/fresh/output")
...
def verify_source():
    ...
    allowed_changes = allocation_product_files | {TEST_FILE, PACKET_README, PACKET_RUNNER}
    if product_changed != allowed_changes:
        raise SystemExit(f"unexpected changes after accepted allocation source: {sorted(product_changed)}")
...
root_config = archived_worker / "vitest.allocation.config.ts"
worker_config = archived_worker / "vitest.allocation.workers.config.ts"
...
export default defineConfig({{
  ...
  test: {{ projects: ["./vitest.allocation.workers.config.ts"] }},
}});
''')
...
def spawn():
    ...
    try:
        ...
        record = {"label": "allocation-fixture", "pid": process.pid, "pgid": process.pid,
