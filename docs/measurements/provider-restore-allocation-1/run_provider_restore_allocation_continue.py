#!/usr/bin/env python3
"""Bounded continuation Vitest runner for deployment realm allocation."""
...
PACKET_README = "docs/measurements/provider-restore-allocation-1/CONTINUATION-README.md"
PACKET_RUNNER = "docs/measurements/provider-restore-allocation-1/run_provider_restore_allocation_continue.py"
FIXTURE_FULL_NAMES = (
...
if len(sys.argv) != 2:
    raise SystemExit("usage: run_provider_restore_allocation_continue.py /absolute/fresh/output")
...
root_config = archived_worker / "vitest.allocation.continue.config.ts"
worker_config = archived_worker / "vitest.allocation.continue.workers.config.ts"
...
export default defineConfig({{
  ...
  test: {{ projects: ["./vitest.allocation.continue.workers.config.ts"] }},
}});
''')
...
source_inputs = [
    *source_files(WORKER / "src"),
    ...
    RUNNER,
    MEASUREMENT_DIR / "CONTINUATION-README.md",
]
...
(OUT / "source-pins.json").write_text(json.dumps({
    ...
    "scope": "five unconsumed public SELF deployment allocation cases; excludes the two prior green assertions",
...
def spawn():
    ...
    try:
        ...
        record = {"label": "allocation-continuation-fixture", "pid": process.pid, "pgid": process.pid,
