...
original_outputs = [
    ...
    for path in json.loads(os.environ.get(
        "PROVIDER_RESTORE_EMPTY_OWNED_PRIOR_OUTPUTS",
        json.dumps([
            str(root / "tmp/provider-restore-empty-owned-1"),
            str(root / "tmp/provider-restore-empty-owned-2"),
            str(root / "tmp/provider-restore-empty-owned-3"),
            str(root / "tmp/provider-restore-empty-owned-4"),
        ]),
    ))
]
if output.exists():
...
test_titles = [
    ...
    "fences normal APIs during a mixed empty and non-empty pending bundle, then completes",
    "replays an empty owned bundle idempotently without changing SQLite state",
    "takes a lease and commits after empty restore, then replay preserves the tip and lease",
    "refuses an existing unmarked empty target and writer, archive, or bundle rebinding",
    "fails closed for zero-byte entries and inconsistent restore count metadata",
]
worker_inputs = [
...
missing_outputs = [path for path in original_outputs if not path.is_dir()]
if missing_outputs:
    raise SystemExit("missing retained prior outputs: " + ", ".join(map(str, missing_outputs)))
original_audits = {str(path): retained_tree(path) for path in original_outputs}
(output / "original-audit.json").write_text(
    json.dumps(original_audits, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
)
...
(output / "source-pins.json").write_text(json.dumps({
    ...
    "expected_test_titles": test_titles,
    "limits": {"test_timeout_seconds": 120, "term_grace_seconds": 5, "kill_grace_seconds": 2},
    "fresh_run_five": True,
    "prior_outputs": [str(path) for path in original_outputs],
    "prior_tree_manifest_sha256": {str(path): audit["manifest_sha256"] for path, audit in original_audits.items()},
    "prior_tree_file_count": {str(path): audit["file_count"] for path, audit in original_audits.items()},
...
(source / "worker/vitest.inventory.workers.config.ts").write_text(
    ...
    f'const baseTest = base.test ?? {{}};\n'
    f'const basePoolOptions = baseTest.poolOptions ?? {{}};\n'
    f'const baseWorkers = basePoolOptions.workers ?? {{}};\n'
    f'export default {{ ...base, cacheDir: {cache_literal}, test: {{ ...baseTest, cache: {{ ...(baseTest.cache ?? {{}}), dir: {cache_literal} }}, poolOptions: {{ ...basePoolOptions, workers: {{ ...baseWorkers, isolatedStorage: false }} }} }} }};\n'
)
(source / "worker/vitest.inventory.config.ts").write_text(
    f'import base from "./vitest.config.ts";\n'
    f'const baseTest = base.test ?? {{}};\n'
    f'const projects = baseTest.projects ?? [];\n'
    f'export default {{ ...base, cacheDir: {cache_literal}, test: {{ ...baseTest, cache: {{ ...(baseTest.cache ?? {{}}), dir: {cache_literal} }}, projects: [projects[0], "./vitest.inventory.workers.config.ts"] }} }};\n'
)
command = [
...
(output / "command.json").write_text(json.dumps({
    ...
    "generated_worker_config_change": "test.poolOptions.workers.isolatedStorage=false; all other base nested options are spread through",
...
try:
    ...
    if isinstance(result, dict):
        for suite in result.get("testResults", []):
            assertion_results.extend(suite.get("assertionResults", []))
    assertion_titles = [item.get("title") for item in assertion_results]
    test_ok = isinstance(result, dict) and result.get("numTotalTests") == 6 and \
        result.get("numPassedTests") == 1 and result.get("numFailedTests") == 0 and \
        result.get("numPendingTests") == 5 and result.get("success") is True and \
        len(assertion_results) == 6 and assertion_titles == test_titles and \
        assertion_results[0].get("status") == "passed" and \
        all(item.get("status") in {"skipped", "pending"} for item in assertion_results[1:])
    cleanup_hold = any(
