...
EXPECTED_SELECTED_COUNT = 23
ORIGINAL_OUT = ROOT / "tmp/workers-pool-1"
ORIGINAL_FILES = (
    "vitest-results.json",
    "verdict.json",
    "native.rc",
...
if len(EXPECTED_CASES) != EXPECTED_RESULT_COUNT or len(SELECTED_CASES) != EXPECTED_SELECTED_COUNT or len(TEST_FILES) != 6:
...
if not ORIGINAL_OUT.is_dir() or any(not (ORIGINAL_OUT / name).is_file() for name in ORIGINAL_FILES):
    raise SystemExit("retained original workers-pool output is incomplete")
original_result = json.loads((ORIGINAL_OUT / "vitest-results.json").read_text())
original_assertions = [
    assertion
    for suite in original_result.get("testResults", [])
    for assertion in suite.get("assertionResults", [])
]
original_names = {assertion.get("fullName") for assertion in original_assertions}
if len(original_assertions) != EXPECTED_RESULT_COUNT or original_names != EXPECTED_CASES:
    raise SystemExit("retained original assertion manifest is not the frozen 43-case result")
original_nonpassed = {assertion["fullName"] for assertion in original_assertions if assertion.get("status") != "passed"}
if original_nonpassed != SELECTED_CASES or sum(assertion.get("status") == "passed" for assertion in original_assertions) != 20:
    raise SystemExit("retained original result does not identify the exact 23-case continuation")
...
selected_pattern = "(?:" + "|".join(re.escape(name) for name in sorted(SELECTED_CASES)) + ")"
vitest_cmd = [str(NODE), str(VITEST), "run", "--config", str(runner_config), "--testNamePattern", selected_pattern, "--reporter=json", "--outputFile", str(OUT / "vitest-results.json"), "--bail=1", *selected]
files = {}
...
for name in ORIGINAL_FILES:
    path = ORIGINAL_OUT / name
    original_hashes[name] = sha256(path)
    add(f"original-output/{name}", path)
...
(OUT / "original-output-sha256.json").write_text(json.dumps(original_hashes, indent=2, sort_keys=True) + "\n")
(OUT / "source-pins.json").write_text(json.dumps({
    ...
    "original_output": str(ORIGINAL_OUT),
    "original_output_files": ORIGINAL_FILES,
...
(OUT / "command.json").write_text(json.dumps({
    ...
    "test_name_pattern": selected_pattern,
}, indent=2, sort_keys=True) + "\n")
...
try:
    ...
    selected_results = [item for item in assertions if item["fullName"] in SELECTED_CASES]
    excluded_results = [item for item in assertions if item["fullName"] in EXPECTED_CASES - SELECTED_CASES]
    result_ok = (
        len(assertions) == EXPECTED_RESULT_COUNT
        and observed_names == EXPECTED_CASES
        and len(selected_results) == EXPECTED_SELECTED_COUNT
        and all(item["status"] == "passed" for item in selected_results)
        and len(excluded_results) == EXPECTED_RESULT_COUNT - EXPECTED_SELECTED_COUNT
    ...
    (OUT / "verdict.json").write_text(json.dumps({"native_rc": native_rc, "verdict_rc": verdict, "reasons": reasons, "assertions": assertions, "expected_case_count": EXPECTED_RESULT_COUNT, "expected_selected_count": EXPECTED_SELECTED_COUNT, "selected_cases": sorted(SELECTED_CASES)}, indent=2, sort_keys=True) + "\n")
